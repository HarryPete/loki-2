...
{  
    ...
    const updateMock = async (id="", status="", type) =>
    {
      try
      {
          setIsLoading(true)
          const url = `/api/batch/${batch._id}`
          const batchDetails = { mock: {quiz:mock}, id, status, type }
          const response = await axios.put(url, batchDetails);
          toast.success(response.data.message);
          getBatch()
      }
    ...
    <Dialog open={selectedMock === batch.mocks[index]?.id} onOpenChange={handleCloseDialog}>
          ...
          <Card className={`${batch.mocks[index] ? 'border-green-500' : 'border-red-500' } border-2 p-4 text-sm`}>
            ...
            <div className='bg-gray-100 p-6 gap-2 flex flex-col items-center rounded relative'>
              ...
              <p>Set {index+1}</p>
              {batch.mocks[index] && <div className="absolute top-2 right-2">
            ...
            <div className="flex justify-center">
              ...
              <Button className='text-xs h-6' onClick={()=> updateMock(batch.mocks[index].id, batch.mocks[index].status === 'Locked' ? 'Unlocked' : 'Locked', "assign")}>Assign</Button>}
            </div>
            </div>}
          </Card>
              ...
              (
                <Card className="flex justify-between items-center text-xs p-4" key={result._id}>
                  <div className="flex items-center gap-1">
                    <Image className="h-5 w-5 rounded-full object-cover object-top" width={100} height={100} src={result.enrollment.user.imageURL} alt={result.enrollment.user.name}/>
